...
impl Player {
    ...
    pub fn update(
        ...
        for (player_entity, player_team, mut player, player_global_tfm, mut player_velocity) in
            players.iter_mut()
        {
            let player_stats = &stats.squads[player.squad as usize];
            ...
            if !player.holding_ball {
                choose_ball_to_chase(&rapier_context, &mut balls, &mut player, player_pos);
            }
            ...
            if accum_linvel.length_squared() > 0.0 {
                ...
                player_velocity.linvel =
                    player_stats.run_speed * player_velocity.linvel.normalize();
            }
...
fn chase_ball(
        ...
        balls.get_mut(chasing_ball_entity)
    {
        ...
        if can_pickup {
            ...
            *accum_linvel += CHASE_FACTOR * player_stats.run_speed * run_direction;
...
fn throw_ball_at_enemy(
        ...
        rapier_context.project_point(player_pos, true, select_enemy_players)
    {
        ...
        if enemy_dist <= player_stats.throw_distance {
            ...
            let throw_velocity = player_stats.throw_speed * throw_dir;
